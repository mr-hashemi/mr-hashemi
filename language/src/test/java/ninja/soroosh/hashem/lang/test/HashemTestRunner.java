...
import ninja.soroosh.hashem.lang.test.HashemTestRunner.TestCase;

public class HashemTestRunner extends ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".hashem";
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        HashemTestSuite suite = c.getAnnotation(HashemTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", HashemTestSuite.class.getSimpleName(), c.getName(), HashemTestRunner.class.getSimpleName()));
        }

        String[] paths = suite.value();

        Class<?> testCaseDirectory = c;
        if (suite.testCaseDirectory() != HashemTestSuite.class) {
    ...
    private static final List<NodeFactory<? extends HashemBuiltinNode>> builtins = new ArrayList<>();
