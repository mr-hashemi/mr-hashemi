...
public class HashemTCKLanguageProvider implements LanguageProvider {
    private static final String ID = "hashemi";
    private static final String PATTERN_VALUE_FNC = "bebin %s() {return %s;}";
    private static final String PATTERN_BIN_OP_FNC = "bebin %s(a,b) {return a %s b;}";
    private static final String PATTERN_POST_OP_FNC = "bebin %s(a) {a %s;}";
    ...
    public Value createIdentityFunction(Context context) {
        return eval(context, "bebin id (a) {return a;}", "id");
    }
    ...
    public Collection<? extends Snippet> createScripts(Context context) {
        final Collection<Snippet> res = new ArrayList<>();
        res.add(loadScript(context, "resources/Ackermann.hashem", TypeDescriptor.NULL, null));
        res.add(loadScript(context, "resources/Fibonacci.hashem", TypeDescriptor.NULL, null));
        return Collections.unmodifiableCollection(res);
    }
    ...
    public Collection<? extends Source> createInvalidSyntaxScripts(Context context) {
        try {
            ...
            res.add(createSource("resources/InvalidSyntax01.hashem"));
            res.add(createSource("resources/InvalidSyntax02.hashem"));
            return Collections.unmodifiableCollection(res);
    ...
    private static Source createSource(final String resourceName) throws IOException {
        ...
        final Reader in = new InputStreamReader(HashemTCKLanguageProvider.class.getResourceAsStream(resourceName), "UTF-8");
