...
public class HashemInstrumentTest {
    ...
    public static class TestRedoIO extends TruffleInstrument {
        ...
        protected void onCreate(Env env) {
            ...
            env.getInstrumenter().attachExecutionEventListener(SourceSectionFilter.ANY, new ExecutionEventListener() {
                @Override
                public void onEnter(EventContext context, VirtualFrame frame) {
                    if ("bekhoon".equals(context.getInstrumentedSourceSection().getCharacters())) {
                        CompilerDirectives.transferToInterpreter();
                        // Interrupt the I/O
    ...
    public void testEarlyReturn() throws Exception {
        String code = "bebin azinja() {\n" +
                ...
                "  ta (a == b && a < 100000) bood {\n" +
                "    a = fce(a);\n" +
                "    b = b + 1;\n" +
                "  }\n" +
                "  c = a;\n" +
                "  // Run fce() and alter it's bede type in an instrument:\n" +
                "  c = fce(c);\n" +
                "  bede c;\n" +
                "}\n" +
                "bebin fce(x) {\n" +
        ...
        earlyReturn.returnValue = new HashemBigNumber(new BigInteger("-42"));
        ret = context.eval(source);
        assertTrue(ret.isNumber());
